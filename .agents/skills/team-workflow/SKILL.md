---
name: team-workflow
description: Claim, split, execute, submit, review, and audit tracked work in the local-agent collaborative delivery PoC.
...
1. Run `whoami`, confirm its Workstation matches `TEAM_WORKFLOW_WORKSTATION_ID`, then run `list`. Treat the returned Role Assignments, not a global job title, as the Account's authorization.
