...
function dashboardHtml(data) {
  ...
  const lanes = roles.map((role) => {
    const cards = data.tasks.filter((task) => task.role === role).map((task) => {
      ...
      const guidance = task.guidanceSnapshot?.sources?.map((source) => `${escapeHtml(source.scope)} v${escapeHtml(source.version)}`).join(', ')
      const evidence = task.evidence ? `<details><summary>Verified Git evidence</summary><p><code>${escapeHtml(task.evidence.commitSha)}</code></p><p>${escapeHtml(task.evidence.pullRequestUrl)}</p><ul>${artifacts}</ul>${guidance ? `<p><strong>Guidance versions:</strong> ${guidance}</p>` : ''}</details>` : '<span class="muted">No verified evidence</span>'
      const parent = task.parentId ? `<div><dt>Parent</dt><dd>${escapeHtml(task.parentId)}</dd></div>` : ''
      return `<article data-task="${escapeHtml(task.id)}"${task.parentId ? ` data-parent="${escapeHtml(task.parentId)}"` : ''}><div class="card-title"><strong>${escapeHtml(task.id)}</strong><em>${escapeHtml(task.status)}</em></div><span>${escapeHtml(task.title)}</span><dl><div><dt>Owner</dt><dd>${escapeHtml(task.ownerId || 'Unassigned')}</dd></div><div aria-label="Reviewer: ${escapeHtml(task.reviewerId)}"><dt>Reviewer</dt><dd>${escapeHtml(task.reviewerId)}</dd></div><div><dt>Requirement:</dt><dd>${escapeHtml(task.requirementId)}</dd></div>${parent}</dl><p>Dependencies: ${dependencies}</p>${task.blockingReason ? `<p class="blocked">${escapeHtml(task.blockingReason)}</p>` : ''}${evidence}</article>`
    }).join('')
    return `<section data-role="${escapeHtml(role)}"><h2>${escapeHtml(roleLabel(role))}</h2>${cards}</section>`
  }).join('')
...
export function buildApp({
  service, users, policies, resolveEffectiveGuidance, webhook, organization, team,
