...
function dashboardHtml(data) {
  ...
  const lanes = roles.map((role) => {
    const cards = data.tasks.filter((task) => task.role === role).map((task) => {
      ...
      return `<article data-task="${escapeHtml(task.id)}"><div class="card-title"><strong>${escapeHtml(task.id)}</strong><em>${escapeHtml(task.status)}</em></div><span>${escapeHtml(task.title)}</span><dl><div><dt>Owner</dt><dd>${escapeHtml(task.ownerId || 'Unassigned')}</dd></div><div aria-label="Reviewer: ${escapeHtml(task.reviewerId)}"><dt>Reviewer</dt><dd>${escapeHtml(task.reviewerId)}</dd></div><div><dt>Requirement:</dt><dd>${escapeHtml(task.requirementId)}</dd></div></dl><p>Dependencies: ${dependencies}</p>${task.blockingReason ? `<p class="blocked">${escapeHtml(task.blockingReason)}</p>` : ''}${evidence}</article>`
    }).join('')
