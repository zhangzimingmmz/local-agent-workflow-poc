...
test('bootstrap configures an isolated port and trusted Project repository without exposing tokens', async (t) => {
  ...
  assert.match(alice, /^TEAM_WORKFLOW_WORKSTATION_ID=workstation-a$/m)
  const bob = await readFile(join(target, 'accounts/bob.env'), 'utf8')
  assert.match(bob, /^TEAM_WORKFLOW_WORKSTATION_ID=workstation-b$/m)
  assert.doesNotMatch(alice, /TEAM_WORKFLOW_SESSION_ID/)
